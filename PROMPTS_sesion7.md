...
|---|---|---|---|---|---|---|---|---|---|
| 7.0 Cierre de la S6 + verificación | ✔ | ✔ | ✔ | — | ✔ | ✔ | — | ✔ | Sonnet |
...
2. **El Prompt 0 es un guardián estricto**: la sesión 6 quedó con la Fase 6.8
   a medio commitear y sin su cierre — el Prompt 0 lo detecta y ORDENA
   terminar eso primero (con los prompts de la propia sesión 6). No se
   empieza la 7 sobre una 6 abierta.
...
### Estado del repositorio al iniciar la sesión (verificado el 2026-08-31)

* Sesión 6 ejecutada hasta la Fase 6.7 + un fix de CI (commits `f335433`…
  `7e5a1b1`); remoto `github.com/growlearnjo/mercadotech`; CI con jobs
  `checks` y `e2e`; `packageManager: npm@11.6.2`.
* **PENDIENTE de la sesión 6**: la Fase 6.8 tiene trabajo sin commitear
  (validator editado, `docs/DEBUGGING.md` sin trackear, retoque en
  `lib/utils.test.ts`) y el Prompt de cierre de la S6 no corrió (la bitácora
  no tiene sección "Sesión 6"; CLAUDE.md dice "Siguiente: sesión 6").
...
## Prompt 0 — Cierre de la sesión 6 y verificación del terreno

```text
[ROL] Actúa como release manager: no abres una iteración nueva sobre otra a
medio cerrar, y lo pruebas con evidencia, no con memoria.

[CONTEXTO] Proyecto MercadoTech, carpeta mercadotech/. Lee CLAUDE.md,
mercadotech/MercadoTech_sesion7.md (secciones "Estado de partida" y
decisión 1) y mercadotech/MercadoTech_sesion6.md (Fase 6.8 y Prompt de
cierre — por si hay que mandarte ahí). Estado esperado del problema: la
Fase 6.8 quedó con cambios sin commitear (.claude/skills/
mercadotech-automatic-validator/SKILL.md modificado, docs/DEBUGGING.md sin
trackear, lib/utils.test.ts retocado) y el cierre de la S6 no corrió (sin
sección "Sesión 6" en docs/BITACORA.md).

[OBJETIVO] En este orden, DETENIÉNDOTE en el primer bloqueo:
1. `git status` y `git log --oneline | head -5`. Si hay trabajo de la 6.8
   sin commitear o la bitácora no tiene sección S6: DETENTE y dime
   exactamente qué prompt de la sesión 6 ejecutar primero (el de la Fase
   6.8 para revisar/commitear lo pendiente, y/o el Prompt de cierre de la
   S6). No los ejecutes tú dentro de este turno.
2. Con la S6 cerrada: verifica el CI en verde (gh no está instalado: dime
   qué mirar en la pestaña Actions de github.com/growlearnjo/mercadotech y
   espera mi confirmación).
3. Verifica localmente: `npm run test` verde con Docker apagado; luego
   `supabase start` + `supabase db reset` + `npm run test:e2e` verde;
   `npm run build` verde.
4. Confirma que NO haya nada nuevo que instalar: esta sesión no agrega
   dependencias (Vercel y Supabase prod se manejan por sus interfaces).

[RESTRICCIONES]
- No escribas código ni documentación de la sesión 7: solo verificación.
- No corras nada contra ningún Supabase remoto.
- Si el CI está rojo, se diagnostica con docs/DEBUGGING.md antes de seguir.

[RAZONAMIENTO] Explica en 2 líneas por qué esta sesión no puede empezar con
la bitácora de la 6 sin escribir (pista: el Prompt de cierre de la 7 diffea
contra el cierre de la 6, y CLAUDE.md guía cada conversación siguiente).

[FORMATO DE SALIDA] (1) Tabla verificación × estado × evidencia; (2) el
veredicto: "S6 cerrada, adelante con la 7" o "ejecutar primero: <prompts>";
(3) sin commit (este prompt no produce cambios).
```
...
lee: mercadotech/MercadoTech_sesion7.md (Fase 7.2 completa y decisiones 3,
4, 5 y 12); next.config.ts (remotePatterns YA correcto — no lo toques);
app/layout.tsx (fuentes YA con next/font); los tres candidatos reales a
dynamic import verificados por la spec: components/chat/ChatWindow (página
/asistente y /soporte), components/seller/OrdersKanban y
SortableImageGallery (páginas del vendedor) — hoy NO existe ningún dynamic
import en el repo; y components/catalog/ProductCard + shared/ProductImage
(ajustes de sizes/priority). La suite de la sesión 6 es tu red: úsala tras
cada cambio.

[OBJETIVO] Ejecuta la Fase 7.2 en tres actos:
...
[FORMATO DE SALIDA] (1) docs/PERFORMANCE.md con metodología + tabla ANTES;
(2) commits de optimización (uno por cambio, con su justificación
numérica); (3) tabla DESPUÉS + puntajes Lighthouse finales; (4) suites
unitaria y E2E verdes al cierre; (5) commits: "perf: <cambio> for Fase
7.2" + "docs: add performance report for Fase 7.2".
```
